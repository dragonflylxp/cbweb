...
class ConnectionPool(object):
    ...
    def check_conn(self, _host, _db, _user, _passwd):
        c = MySQLdb.connect(host=_host, db=_db, user=_user, passwd=_passwd)
        c.close()
...
class PoolManager(object):
    ...
    def set_up(cls, cfg):
        ...
        for db, cfg in PoolManager._cfg.iteritems():
            _host = cfg.get("host")
            _db = cfg.get("db")
            ...
            pool = ConnectionPool(_host, _db, _charset, _user, _passwd, _cached)
