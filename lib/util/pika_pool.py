...
# date: 18-6-6
# file: pika_pool.py
# desc: https://github.com/bninja/pika-pool 本来是pypi的库，但是因为pika_pool两年没更新了，
#       而且只适配到pika-0.10.0,为了兼容py3和pika新版本，将主逻辑移过来
...
class QueuedPool(Pool):
    ...
    class Fairy(Pool.Fairy):
        ...
        def __init__(self, cxn):
            ...
            self.released_at = self.created_at = time()
    ...
    def is_stale(self, fairy):
        if not self.stale:
            return False
        return (time() - fairy.released_at) > self.stale

    def is_expired(self, fairy):
        if not self.recycle:
            return False
        return (time() - fairy.created_at) > self.recycle
